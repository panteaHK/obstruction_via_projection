...
def make_rectangle_lim(limx, limy, n_samples=500, color=0):
    data = np.zeros((n_samples,3))
    for i in range(n_samples):
        x, y = np.random.uniform(limx[0], limx[1]), np.random.uniform(limy[0], limy[1])
        data[i, 0], data[i, 1], data[i, 2] = x, y, color
    return data
...
def sample_blue_points_checkerboard(dim, n_samples, percentage):
    output = np.empty((0,3))
    step = percentage/200
    for i in range(n_samples):
        x, y = np.random.uniform(0, 8, 2)
        fx = int(np.floor(x))
        fy = int(np.floor(y))
        if (fx + fy) % 2 == 1:
            ...
            if cy != 8:
                margins.append(cy-y)
            if np.min(margins) <= step:
                output = np.vstack((output, np.array([x, y, 0])))
    return output * (dim /8)


def sample_red_points_checkerboard(dim, n_samples, percentage):
    output = np.empty((0,3))
    step = percentage/200
    for i in range(n_samples):
        x, y = np.random.uniform(0, 8, 2)
        ...
        fy = int(np.floor(y))
        if (fx + fy) % 2 == 0:
            output = np.vstack((output, np.array([x*dim/8, y*dim/8, 1])))
        else:
            cx, cy = np.ceil(x), np.ceil(y)
            margins = []
            if fx != 0:
...
def geometric_mixing_checkerboards_fixed(dim, n_samples, image_output = False, trial_range=(0, 1)):
    color_n_samples = int(np.round(n_samples/2))
    for i in range(11):
        for j in range(*trial_range):
            ...
            reds = sample_red_points_checkerboard(dim, color_n_samples, i*10)
            blues = sample_blue_points_checkerboard(dim, color_n_samples, i*10)
            data = np.vstack((reds, blues))
            np.save('pointsets/checkerboard_gm_fixed/checkerboard_gm_fixed_{}_{}.npy'.format(i * 10, j), data)
        if image_output:
            plt.tick_params(
...
def geometric_mixing_checkerboards(dim, density, image_output=False, trial_range=(0, 1)):
    unit = dim/8
    ...
    for i in range(11):
        for j in range(*trial_range):
            ...
            np.save('pointsets/checkerboard_gm/checkerboard_gm_{}_{}.npy'.format(i * 10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_cross(x1, x2, x3, x4, y1, y2, y3, y4, density, image_output=False, trial_range=(0, 1)):
    y_m = (y3+y2)/2
    x_m = (x3+x2)/2
    for i in range(0,11):
        for k in range(*trial_range):
            ...
            data = np.vstack((reds, blues))
            np.save('pointsets/cross_gm/cross_gm_{}_{}.npy'.format(i*10, k), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_cross_fixed(x1,x2,x3,x4,y1,y2,y3,y4, n_samples, image_output=False,trial_range=(0, 1)):
    y_m = (y3 +y2)/2
    x_m = (x3+x2)/2
    for i in range(0,11):
        for k in range(*trial_range):
            ...
            for j in range(int(n_samples/2)):
                ...
                blues = np.vstack((blues, np.array([[rx, ry, 0], [lx, ly, 0]])))
            data = np.vstack((reds, blues))
            np.save('pointsets/cross_gm_fixed/cross_gm_fixed_{}_{}.npy'.format(i*10, k), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_circles(c1, c2, r, n_samples, image_output=False, trial_range=(0, 1)):
    ...
    for i in range(11):
        for j in range(*trial_range):
            ...
            data = np.vstack((reds, blues))
            np.save('pointsets/circles_gm/circles_gm_{}_{}.npy'.format(i * 10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_inner_circles_fixed(r_inner, r_outer, n_samples, image_output=False, trial_range=(0, 1)):
    ...
    for i in range(0,11):
        for j in range(*trial_range):
            ...
            np.save('pointsets/inner_circle_gm_fixed/inner_circle_gm_fixed_{}_{}.npy'.format(i*10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_inner_circles(r_inner, r_outer, density, image_output=False, trial_range=(0, 1)):
    red_step = (r_outer - r_inner)/10
    blue_step = r_inner/10
    for i in range(0,11):
        for j in range(*trial_range):
            ...
            data = np.vstack((red, blue))
            np.save('pointsets/inner_circle_gm/inner_circle_gm_{}_{}.npy'.format(i*10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_s_curve(n_samples, noise, image_output=False, trial_range=(0, 1)):
    for i in range(11):
        for j in range(*trial_range):
            ...
            for p in blue_extra:
                ...
                if p[1] < 0:
                    if (p[1] - b_min) / (- b_min) < i / 10:
                        p[-1] = 0
                        data = np.vstack((data, p))
            np.save('pointsets/s_curve_gm/s_curve_gm_{}_{}.npy'.format(i*10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_crossing_recs_fixed(x_start, x_end, y_start, y_end, n_samples, image_output=False, trial_range=(0, 1)):
    ...
    for i in range(11):
        for j in range(*trial_range):
            ...
            np.save('pointsets/crossing_recs_gm_fixed/crossing_recs_gm_fixed_{}_{}.npy'.format(i * 10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def geometric_mixing_crossing_recs(x_start, x_end, y_start, y_end, density, image_output=False, trial_range=(0, 1)):
    right_step = (x_end[1] - x_start[1])/10
    left_step = (x_start[0] - x_end[0])/10
    up_step = (y_end[1] - y_start[1])/ 10
    down_step = (y_start[0] - y_end[0])/10
    ...
    for i in range(11):
        for j in range(*trial_range):
            ...
            blues2 = make_rectangle_lim(limx=(x_start[1] - i * in_blue_step, w),
                                        ...
                                        color=0)
            data = np.vstack((blues1, blues2,reds))
            np.save('pointsets/crossing_recs_gm/crossing_recs_gm_{}_{}.npy'.format(i * 10, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_inner_circle(r_inner, r_outer, n_samples, image_output=False, trial_range=(0, 1)):
    for i in range(0, 80, 10):
        for j in range(*trial_range):
            red = make_circle(n_samples=n_samples, radius=r_inner, color=1)
            blue = make_2d_donut(n_samples, r_inner, r_outer, color=0)
            data = np.vstack((red, blue))
            data = randomize_data(data, i)
            np.save('pointsets/inner_circle_pm/inner_circle_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
                ...
                labelbottom=False,
                labelleft=False)
            util.plot_points_2d_np(data, plt, 'pointsets/inner_circle_pm/inner_circle_pm_{}.png'.format(i))


def probabilistic_mixing_checkerboard(n_samples, dim, image_output=False, trial_range=(0, 1)):
    for i in range(0, 80, 10):
        for j in range(*trial_range):
            data = make_checkerboard(n_samples, dim)
            data = randomize_data(data, i)
            np.save('pointsets/checkerboard_pm/checkerboard_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_circles(c1, c2, r, n_samples, image_output=False, trial_range=(0, 1)):
    c1 = np.array([c1[0], c1[1], 0])
    c2 = np.array([c2[0], c2[1], 0])
    for i in range(1,11):
        for j in range(*trial_range):
            ...
            data = np.vstack((reds, blues))
            data = randomize_data(data, i)
            np.save('pointsets/circles_pm/circles_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_cross(lim1x, lim2x, lim1y, lim2y, n_samples, image_output=False, trial_range=(0, 1)):
    for i in range(0,80,10):
        for j in range(*trial_range):
            data = make_cross(lim1x, lim2x, lim1y, lim2y, n_samples)
            data = randomize_data(data, i)
            np.save('pointsets/cross_pm/cross_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_s_curve(n_samples, noise, image_output=False, trial_range=(0, 1)):
    for i in range(0, 80, 10):
        for j in range(*trial_range):
            data = make_s_dataset(n_samples, noise)
            data = randomize_data(data, i)
            np.save('pointsets/s_curve_pm/s_curve_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_rectangles(x_lim_v, y_lim_v, x_lim_h, y_lim_h, n_samples, image_output=False, trial_range=(0,1)):
    for i in range(0, 80, 10):
        for j in range(*trial_range):
            ...
            data = np.vstack((blues1, blues2, reds))
            data = randomize_data(data, i)
            np.save('pointsets/crossing_recs_pm/crossing_recs_pm_{}_{}.npy'.format(i,j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def probabilistic_mixing_moons(n_samples, noise, image_output=False, trial_range=(0, 1)):
    for i in range(0, 80, 10):
        for j in range(*trial_range):
            ...
            if minr > minb:
                for p in data:
                    p[2] = (p[2] + 1) % 2
            data = randomize_data(data, i)
            np.save('pointsets/moons_pm/moons_pm_{}_{}.npy'.format(i, j), data)
        if image_output:
            plt.tick_params(
                axis='both',  # changes apply to the x-axis
                which='both',  # both major and minor ticks are affected
...
def generate_probabilistic_datasets(trial_range):
    # probabilistic_mixing_inner_circle(2, 4, 1500, trial_range=trial_range)
    # probabilistic_mixing_checkerboard(5000, 1, trial_range=trial_range)
    # probabilistic_mixing_circles((0, 0), (2.5, 0), 1, 1000, trial_range=trial_range)
    # probabilistic_mixing_cross((0, 3), (4, 7), (0, 3), (4, 7), 700, trial_range=trial_range)
    # probabilistic_mixing_s_curve(1500, 0.05, trial_range=trial_range)
    # probabilistic_mixing_rectangles((1.75, 2.25), (0, 8), (0, 4), (1.5, 6.5), 1500, trial_range=trial_range)
    # probabilistic_mixing_moons(2000, 0.05, True, trial_range=trial_range)
    # print("ended the probabilistic mixing")
    # geometric_mixing_inner_circles(2, 4, 75, trial_range=trial_range)
    # geometric_mixing_inner_circles_fixed(2, 4, 1500, trial_range=trial_range)
    # print("ended the geometric inner circles")
    # geometric_mixing_checkerboards_fixed(1, 7000, trial_range=trial_range)
    # geometric_mixing_checkerboards(8, 70, trial_range=trial_range)
    # print("ended the geometric checkerboard")
    geometric_mixing_circles(c1=(0, 0), c2=(2.5, 0), r=1, n_samples=1000, image_output=True, trial_range=trial_range)
    # print("ended the geometric circles")
    # geometric_mixing_cross_fixed(0, 3, 4, 7, 0, 3, 4, 7, 1400, trial_range=trial_range)
    # geometric_mixing_cross(0, 3, 4, 7, 0, 3, 4, 7, 150, trial_range=trial_range)
    # print("ended the geometric cross")
    # geometric_mixing_s_curve(1000, 0.05, trial_range=trial_range)
    # print("ended the geometric S curve")
    # geometric_mixing_crossing_recs((1.75, 2.25), (0, 4), (1.5, 6.5), (0, 8), 100, trial_range=trial_range)
    # geometric_mixing_crossing_recs_fixed((1.75, 2.25), (0, 4), (1.5, 6.5), (0, 8), 1500, trial_range=trial_range)
    # print("ended the geometric rectangles")
